...
class Data:
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
                 edge_ids: np.ndarray, labels: np.ndarray, raw_edge_ids: np.ndarray = None):
        """
...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    if dataset_name in ['enron']:
        dt_path = './processed_data/{}/processed_{}.csv'.format(dataset_name, dataset_name)
    else:
        dt_path = './processed_data/{}/ml_{}.csv'.format(dataset_name, dataset_name)
    if os.path.exists(dt_path):
        if dataset_name in ['enron']:
            graph_df = pd.read_csv('./processed_data/{}/processed_{}.csv'.format(dataset_name, dataset_name))
            edge_raw_features = np.load(
                './processed_data/{}/processed_{}.npy'.format(dataset_name, dataset_name))
        else:
