...
class ConfigSaving:
    def __init__(self, model_name, config_name, args, save_dir):
    ...
    def save_config(self):
        config_path = os.path.join(self.save_dir, f"{self.config_name}.json")
        with open(config_path, "w") as f:
            json.dump(vars(self.args), f, indent=4)
        print(f"Config saved to {config_path}")

    def record(self, metrics, stage, pre_fix=None):
        if pre_fix:
            key = f"{pre_fix}_stage_{stage}"
        else:
            key = f"stage_{stage}"
    ...
    def end(self):
        metrics_path = os.path.join(self.save_dir, f"{self.config_name}_metrics.json")
...
class NegativeEdgeSampler(object):
    ...
    def random_sample_with_collision_check(self, size: int, batch_src_node_ids: np.ndarray,
                                           batch_dst_node_ids: np.ndarray, max_trials: int = 10_000):
        """
        ...
        src_pool = list(self.unique_src_node_ids)
        dst_pool = list(self.unique_dst_node_ids)

        num_trials = 0
        while len(neg_src_ids) < size and num_trials < max_trials:
            src = self.random_state.choice(src_pool)
            dst = self.random_state.choice(dst_pool)
            if (src, dst) not in batch_edges:
                neg_src_ids.append(src)
                neg_dst_ids.append(dst)
            else:
                num_trials += 1

        if len(neg_src_ids) < size:
            raise RuntimeError(f"Unable to sample {size} negative edges after {max_trials} trials.")

        return np.array(neg_src_ids), np.array(neg_dst_ids)
...
def structural_similarity(g1, g2):
    """Calculate the structural similarity of adjacent blocks"""
    common_nodes = find_common_nodes(g1, g2)
    deg1 = g1.in_degrees(common_nodes['g1_idx'])
    deg2 = g2.in_degrees(common_nodes['g2_idx'])
    deg_diff = torch.mean(torch.abs(deg1 - deg2) / (deg1.float().mean() + 1e-8))
    return 1.0 - deg_diff.item()


def find_common_nodes(g1, g2):
    """Find the mapping of common node indices of the two graphs"""
    nodes1 = g1.nodes().tolist()
    nodes2 = g2.nodes().tolist()
    common = list(set(nodes1) & set(nodes2))
    g1_idx = torch.tensor([nodes1.index(n) for n in common], dtype=torch.long)
    g2_idx = torch.tensor([nodes2.index(n) for n in common], dtype=torch.long)
    return {'g1_idx': g1_idx, 'g2_idx': g2_idx}


def compute_node_smoothness_loss(blocks, block_reprs_list, base_lambda=0.2, max_lambda=0.5):
    """compute node smoothness loss """
    total_loss = 0.0

    for b in range(1, len(block_reprs_list)):
        # Obtain the graphs and representations of adjacent blocks
        prev_repr = block_reprs_list[b - 1]
        current_repr = block_reprs_list[b]

        prev_block = blocks[b - 1]
        current_block = blocks[b]

        sim = structural_similarity(prev_block, current_block)

        # Dynamic weights: The greater the structural change, the weaker the regularization.
        lambda_t = base_lambda * sim
        lambda_t = torch.clamp(torch.tensor(lambda_t, dtype=torch.float64), max=max_lambda)

        # Huber loss
        huber_loss = nn.HuberLoss(delta=0.3)(current_repr, prev_repr)

        total_loss += lambda_t.item() * huber_loss.item()

    return torch.tensor(total_loss / (len(block_reprs_list) - 1), dtype=torch.float64, requires_grad=True)


def create_temporal_blocks(dataset_name, node_raw_features, edge_raw_features, full_data, stride, block_size, device = 'cpu', drop_last=False, evaluate=False):
    """
    Generate time blocks based on the sliding window of edge quantities
    """
    ...
    edge_feat_dim = edge_raw_features.shape[1] # edge_feat_dim = node_feat_dim
    ...
    random_node_features = torch.empty(num_nodes, node_feat_dim)

    if dataset_name not in ['USLegis']:
        if evaluate:
            random_node_features = torch.tensor(np.load(f'./processed_data/random_node_features/{dataset_name}.npy'))
        else:
            init.xavier_uniform_(random_node_features)
            np.save(f'./processed_data/random_node_features/{dataset_name}.npy', random_node_features.cpu().numpy())

    print(random_node_features)
    ...
    for start_idx in range(0, n, stride):
        ...
        block_src_node_ids = src_node_ids[start_idx : end_idx]
        block_dst_node_ids = dst_node_ids[start_idx : end_idx]
        block_edge_ids = edge_ids[start_idx : end_idx]
        block_node_interact_times = node_interact_times[start_idx : end_idx]


        block_node_features = node_raw_features
        block_edge_features = edge_raw_features[start_idx: end_idx] # [E, D]


        start_time = block_node_interact_times[0]
        end_time = block_node_interact_times[-1]


        graph = dgl.graph((torch.tensor(block_src_node_ids) - 1, torch.tensor(block_dst_node_ids) - 1),
                          num_nodes=num_nodes)
        # graph = dgl.to_simple(graph)
        # graph = dgl.to_bidirected(graph)
        graph.add_edges(torch.tensor(block_dst_node_ids) - 1, torch.tensor(block_src_node_ids) - 1)

        if dataset_name in ['USLegis']:
            # Generate node features based on edge features

            all_nodes = np.concatenate([block_src_node_ids, block_dst_node_ids])  # [2E,]
            all_feats = np.concatenate([block_edge_features, block_edge_features])  # [2E, D]

            sum_matrix = np.zeros((num_nodes + 1, node_feat_dim), dtype=np.float32)
            count_matrix = np.zeros(num_nodes + 1, dtype=np.int32)

            np.add.at(sum_matrix, all_nodes, all_feats)
            np.add.at(count_matrix, all_nodes, 1)

            avg_matrix = np.divide(
                sum_matrix,
                count_matrix[:, None],
                out=np.zeros_like(sum_matrix),
                where=count_matrix[:, None] > 0
            )

            block_node_features = avg_matrix
            block_node_features = block_node_features[1:, :]

            print(block_node_features)
        else:
            block_node_features = random_node_features

        graph.ndata['feat'] = torch.tensor(block_node_features)
    ...
    return blocks, start_times, end_times
